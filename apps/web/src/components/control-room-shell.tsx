...
export async function ControlRoomShell({ active, eyebrow, title, actions, children }: { active: string; eyebrow: string; title: string; actions?: ReactNode; children: ReactNode }) {
  ...
  const agentLabel = health?.agent.configured
    ? `${health.agent.provider} ready · ${health.agent.model}`
    : health ? `${health.agent.provider} agent needs configuration` : "Start the API on port 4000";
  return (
    <main className="control-room">
      <aside className="sidebar">
        ...
        <div className="sidebar-foot"><span className={health ? "status-dot online" : "status-dot"} /><div><strong>{health ? "Systems operational" : "API unavailable"}</strong><span>{health ? agentLabel : "Start the API on port 4000"}</span></div></div>
      </aside>
